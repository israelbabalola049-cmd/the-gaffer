...
const generateFixtures = (myClub, allClubs) => {
  ...
  // Home and away vs every league club
  leagueClubs.forEach(opponent => {
    fixtures.push({
      ...
      homeClub: myClub,
      awayClub: opponent,
      venue: myClub.stadium || `${myClub.name} Stadium`,
      played: false,
      result: null,
    });
    week++;
    ...
    fixtures.push({
      ...
      homeClub: opponent,
      awayClub: myClub,
      venue: opponent.stadium || `${opponent.name} Stadium`,
      played: false,
      result: null,
    });
    week++;
  ...
  // Domestic cup fixtures (rounds 1-4)
...
const simulateMatch = (homeClub, awayClub, allPlayers) => {
  ...
  const homeRating = getSquadRating(homeClub.name) + randomBetween(0, 8); // home advantage
...
const generateYouthPlayers = (myClub) => {
  ...
  return Array.from({ length: 8 }, (_, i) => ({
    ...
    overall: randomBetween(55, 68),
    potential: randomBetween(72, 88),
    age: randomBetween(16, 19),
    pace: randomBetween(60, 82),
    shooting: randomBetween(50, 75),
    passing: randomBetween(55, 78),
    dribbling: randomBetween(58, 80),
    defending: randomBetween(45, 72),
    physical: randomBetween(55, 75),
    value: randomBetween(500000, 3000000),
    wage: randomBetween(1000, 8000),
...
const useGameStore = create(
  persist(
    (set, get) => ({
      ...
      /* ── chooseClub — now initialises all new state too ── */
      chooseClub: (club) => {
      ...
      buyPlayer: (player, fee) =>
        set(state => {
          ...
          const morale = { ...state.morale, [player.id]: 75 };
          return {
      ...
      // Play a fixture — simulate result, update table, log result, advance week
      playFixture: (fixtureId) => {
        ...
        const fixtures = state.fixtures.map(f =>
          f.id === fixtureId
            ? { ...f, played: true, result: { homeGoals, awayGoals } }
            : f
        );

        // Update league table (league fixtures only)
        ...
        if (fixture.competitionType === 'league') {
          leagueTable = leagueTable.map(row => {
            ...
            const formResult = won ? 'W' : drw ? 'D' : 'L';
            return {
              ...row,
              played: row.played + 1,
              won:    row.won  + (won ? 1 : 0),
              ...
              form: [...row.form.slice(-4), formResult],
            };
          }).sort((a, b) => b.points - a.points || b.goalDifference - a.goalDifference);
        }

        // Manager rating shift
        ...
        const nextFixtureIndex = state.currentFixtureIndex + 1;
      ...
      // Simulate all remaining fixtures (auto result)
      simRemaining: () => {
        const state = get();
        const unplayed = state.fixtures.filter(f => !f.played);
        unplayed.forEach(f => get().playFixture(f.id));
      },
      ...
      addNotification: (notification) =>
        set(state => ({
          notifications: [
            { id: Date.now(), read: false, week: state.week, ...notification },
            ...state.notifications,
          ],
        })),
      ...
      addInjury: (playerId, recoveryWeeks) =>
        set(state => ({
          injuries: [...state.injuries, { playerId, recoveryWeeks, weekInjured: state.week }],
        })),

      removeInjury: (playerId) =>
        set(state => ({
          injuries: state.injuries.filter(i => i.playerId !== playerId),
        })),

      // Morale
      setPlayerMorale: (playerId, value) =>
        set(state => ({
          morale: { ...state.morale, [playerId]: Math.min(100, Math.max(0, value)) },
        })),

      // Objectives
      markObjectiveMet: (id) =>
        set(state => ({
          objectives: state.objectives.map(o => o.id === id ? { ...o, met: true } : o),
        })),

      // Awards
      addAward: (award) =>
        set(state => ({
          awards: [...state.awards, { ...award, season: state.season, week: state.week }],
        })),

      // Career history — call at end of season
      recordSeasonEnd: () => {
        const state = get();
